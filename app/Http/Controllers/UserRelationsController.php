...
class UserRelationsController extends Controller
{
    ...
    public function setDistCompanies(Request $request)
    {
        ...
        $selected = DB::table('dists_comps')->where('dist_id', request()->id)->pluck('comp_id')->toArray();
        $comps = \Auth::user()->parents->where('user_category_id', '=', 1);
        // dd($comps, $non_allowed_ids);
        return view('userRelations.distCompanies', ['dist' => $dist, 'comps' => $comps, 'selected' => $selected]);
    }
        ...
        if ($request['comps'] != null) {
            foreach ($request['comps'] as $comp) {
                DB::table('dists_comps')->insert(
                    ['dist_id' => request()->id, 'comp_id' => $comp]
                );
            }
        }
    ...
    public function addFavAg()
    {
        ...
        foreach ($agents->where('user_category_id', 2) as $child) foreach ($child->children as $chd)
    ...
    protected function ajaxFavAgRequest(Request $request)
    {
        try {
            ...
            foreach ($comp->children->where('user_category_id', 2) as $child) foreach ($child->children as $chd)
