                ...
                @foreach ($offers as $offer) 
                    <tr>
                        ...
                        <td>
                            ...
                            @if (Auth::user()->user_category_id == 0 || Auth::user()->id == $offer->user_id)
                                {{-- <a class="btn btn-secondary btn-sm" href="{{ route('editOffer', ['id'=>$offer->id, 'type'=>1]) }}">{{__('Edit')}}</a> --}}
                                <form action="{{route('deleteOffer',['id'=>$offer->id, 'type'=>1])}}" method="POST" class="d-inline-block">
                                    @csrf
            ...
            <table class="table table-hover table-sm table-bordered nowrap mt-2 accordion" id="basketsTable">
                ...
                @foreach ($baskets as $basket)
                        ...
                        <td>
                            ...
                            @if (Auth::user()->user_category_id == 0 || Auth::user()->id == $basket->user_id)
                                {{-- <a class="btn btn-secondary btn-sm" href="{{ route('editOffer', ['id'=>$basket->id, 'type'=>2]) }}">{{__('Edit')}}</a> --}}
                                <form action="{{route('deleteOffer',['id'=>$basket->id, 'type'=>2])}}" method="POST" class="d-inline-block">
                                    @csrf
                                    <input type="hidden" name="_method" value="DELETE">
                                    <button class="btn btn-danger btn-sm" type="submit">
                                        {{__("Delete")}}
