...
@section('content')
    ...
    <div class="row m-0 col-12 mt-3">
        <div class="col-md-8 offset-md-2 p-4">
                <h3 class="w-50 d-inline-block">{{__('Items')}}&nbsp;{{__('By Agent')}}
                    <select id="reciever_id" class="form-control form-control-sm" name="reciever_id" value="" onchange="agentPageChange(this)" required>
                        <option value="0">{{__('all')}}</option>
                        @foreach ($agents as $agent)
                            <option value="{{$agent->id}}" @if($agent->id==$agent_id) selected @endif >{{$agent->f_name}}&nbsp;@if($agent->s_name != null){{$agent->s_name}}@endif</option>
                        @endforeach
                    </select>
                </h3>
                    @if (Auth::user()->user_category_id == 0 || Auth::user()->user_category_id == 1 || Auth::user()->user_category_id == 4)
                        ...
                        <button class="btn btn-info float-right mx-2" type="submit" onclick="window.location='{{route('createItem')}}'">
                            {{__("Create Item")}}
                        </button>
                    ...
                    @endif
                    @if(Auth::user()->user_category_id == 0 || Auth::user()->user_category_id == 2 || Auth::user()->user_category_id == 3)
                        <button class="btn btn-secondary float-right" type="submit" onclick="window.location='{{route('offers')}}'">
                            {{__("Offers")}}
                        </button>
                    @endif
                <hr>
            ...
            <table class="table table-hover table-sm table-bordered nowrap mt-2" id="ItemsTable">
                ...
                @foreach ($items as $item) 
                    <tr @if($item->isFreezedByUser(Auth::user()->id)) class='table-danger' @endif >
                        ...
                        @endif
                        <td>
                            <a class="btn btn-primary btn-sm" href="{{ route('showItem', ['id'=>$item->id]) }}" title="{{__('Show')}}"><i class="fas fa-eye"></i></a>
                            @if (Auth::user()->user_category_id == 0 || Auth::user()->id == $item->user_id)
                                <a class="btn btn-secondary btn-sm" href="{{ route('editItem', ['id'=>$item->id]) }}" title="{{__('Edit')}}"><i class="fas fa-edit"></i></a>
                                <form action="{{route('deleteItem',['id' => $item->id])}}" method="POST" class="d-inline-block">
                                    @csrf
                            ...
                            @elseif (Auth::user()->user_category_id == 5)
                                <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#sender_remark" title="{{__('Insert Sender Remark')}}"  onclick="sendItemsToCart({{$item->id}}, false)">
                                    <i class="fas fa-comment"></i>
                                </button>
                                <button type="button" class="btn btn-primary disabled btn-sm" title="{{__('Add to Cart')}}"  onclick="ajaxSendtoCart({{$item->id}}, false)" disabled id="i_btn{{$item->id}}">
                            ...
                            @endif
                            @if((Auth::user()->user_category_id == 2 || Auth::user()->user_category_id == 3 || Auth::user()->user_category_id == 4) && ! $item->isFreezedByUser(Auth::user()->id))
                            <button type="button" class="btn btn-danger btn-sm" onclick="freezeItemByAgent({{$item->id}}, true)" title="{{__('Freeze')}}">
                                <i class="far fa-stop-circle"></i>
                            </button>
                            @elseif((Auth::user()->user_category_id == 2 || Auth::user()->user_category_id == 3 || Auth::user()->user_category_id == 4) && $item->isFreezedByUser(Auth::user()->id))
                            <button type="button" class="btn btn-info btn-sm" onclick="freezeItemByAgent({{$item->id}}, false)" title="{{__('Unfreeze')}}">
                                <i class="far fa-stop-circle"></i>
                            </button>
                            @endif
                        </td>
                                ...
                                <td @if(Auth::user()->user_category_id == 5) colspan="7" @else colspan="5" @endif >
                                    <div class="collapse" id="collapseBItems{{$basket->id}}">
                                        @foreach ($basket->items as $item)
                                            {{$item->name ." ". $item->type->ar_name ." ". $item->titer}} : <b>{{$item->pivot->quantity}}</b>
                                        <br>                                
                                        @endforeach
                                    </div>
                                </td>
    ...
    <div class="modal fade" id="sender_remark" tabindex="-1" role="dialog" aria-labelledby="sender_remarkLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <button type="button" class="btn btn-primary" onclick="ajaxSendtoCart()">{{__('Add to Cart')}}</button>
            </div>
        </div>
        </div>
    </div>
          ...
          function sendAllToCart(){
            $.ajaxSetup({
              ...
              $.each( $('.q_inp'), function() {
                  ...
                  if(this.value > 0)
                  {
                      var obj = {id: id, quant: this.value, reciever: $("#reciever_id").val()};
                      objects.push(obj);
                  }
                console.log(objects);
              });
