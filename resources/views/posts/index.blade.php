                    ...
                    <div class="row no-gutters mh-200">
                        ...
                        <div class="col-6">
                            <div class="card-body">
                                ...
                                <p class="card-text"><small class="text-muted">{{ __('Last updated') }}
                                        {{ $post->updated_at->diffForHumans() }}</small></p>
                            </div>
                        </div>
                        @auth
                            <div class="col-2">
                                @if (Auth::user()->user_category_id == 6 || Auth::user()->id == $post->author->id)
                                    <a class="btn btn-secondary" href="{{ route('editPost', ['id' => $post->id]) }}"
                                        style="margin-top:40%">{{ __('Edit') }}</a>
                                @endif
                            </div>
                        @endauth
                    </div>
