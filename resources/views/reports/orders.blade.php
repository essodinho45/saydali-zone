                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="POST" action="{{ route('updateOrdersReport') }}" enctype="multipart/form-data">
                                @csrf
                                @if (Auth::user()->user_category_id == 6 || Auth::user()->user_category_id == 2)
                                    <div class="form-group row">
                                        {{-- <label for="country" class="col-md-4 col-form-label text-md-right">{{ __('Country') }}</label> --}}
                                        @if (Auth::user()->user_category_id == 6)
                                            <label class="col-md-3 text-right" for="agent">{{ __('Agent') }} *</label>
                                        @else
                                            <label class="col-md-3 text-right" for="agent">{{ __('Distributor') }}
                                                *</label>
                                        @endif
                                        <div class="col-md-6">
                                            <select class="form-control" id="agentFavAg"
                                                ...
                                                @foreach ($agents as $agent)
                                                    <option value="{{ $agent->id }}"
                                                        @if ($selected_agent == $agent->id) selected @endif>
