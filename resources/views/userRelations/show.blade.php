                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-8">
                                ...
                                <hr>
                                @if ($user->tel1 != null)
                                    <h5><i class="fas fa-phone mr-3"></i>{{ $user->tel1 }}&nbsp;@if ($user->tel2 != null)
                                            - {{ $user->tel2 }}
                                        @endif
                                    </h5>
                                @endif
                                @if ($user->mob1 != null)
                                    <h5><i class="fas fa-mobile-alt mr-4"></i>{{ $user->mob1 }}&nbsp;@if ($user->mob2 != null)
                                            - {{ $user->mob2 }}
                                        @endif
                                    </h5>
                                @endif
                                @if ($user->fax1 != null)
                                    <h5><i class="fas fa-fax mr-3"></i>{{ $user->fax1 }}&nbsp;@if ($user->fax2 != null)
                                            - {{ $user->fax2 }}
                                        @endif
                                    </h5>
                                @endif
                                <h5><i class="fas fa-at mr-3"></i>{{ $user->email }}&nbsp;@if ($user->email2 != null)
                                        - {{ $user->email2 }}
                                    @endif
                                </h5>
                                <h5><i
                                        class="fas fa-map-marker-alt mr-4"></i>{{ $user->itsCountry->ar_name }}&nbsp;-&nbsp;{{ $user->itsCity->ar_name }}
                                    @if ($user->region != null)
                                        &nbsp;-&nbsp;{{ $user->region }}
                                        @endif @if ($user->address != null)
                                            &nbsp;-&nbsp;{{ $user->address }}
                                        @endif
                                </h5>
                                @if ($user->user_category_id == 2)
                                    ...
                                    <ul>
                                        @foreach ($user->children as $parent)
                                            <li><a href="{{ route('showUsr', [$parent->id]) }}">{{ $parent->f_name }}
                                                    &nbsp; {{ $parent->s_name }}</a></li>
                                        @endforeach
                                    </ul>
                                @endif
                        ...
                        @auth
                            @if ($user->user_category_id == 2 && Auth::user()->category->id == 6)
                                <hr>
                            ...
                            @endif
                            @if (Auth::user()->category->id == 6 || Auth::user()->category->id == 5)
                                <hr>
                                @if (Auth::user()->category->id == 6)
                                    @if ($user->email_verified_at == null)
                                        <a class="btn btn-info float-right mx-1" id="verBtn{{ $user->id }}" href="#"
                                            onclick="verUsrAjax({{ $user->id }})">{{ __('Verify') }}</a>
                                    @endif
                                    @if ($user->user_category_id != 6)
                                        @if ($user->freezed == false)
                                            <a class="btn btn-danger float-right mx-1" id="frzUsrBtn{{ $user->id }}"
                                                href="#"
                                                onclick="frzUsrAjax({{ $user->id }})">{{ __('Freeze') }}</a>
                                        @elseif($user->freezed == true)
                                            <a class="btn btn-secondary float-right mx-1" id="unfrzUsrBtn{{ $user->id }}"
                                                href="#"
                                                onclick="unfrzUsrAjax({{ $user->id }})">{{ __('Unfreeze') }}</a>
                                        @endif
                                        @if (
                                            $user->items->count() == 0 &&
                                            ...
                                            <form action="{{ route('deleteUser', ['id' => $user->id]) }}" method="POST"
                                                class="d-inline-block float-right mx-1">
                                                @csrf
                                ...
                                @if ($user->category->id == 3 || $user->category->id == 2)
                                    <a class="btn btn-info float-right mx-1" id="viewItemsBtn{{ $user->id }}"
                                        href="{{ route('itemsByAgent', [$user->id]) }}">{{ __('Show Items') }}</a>
                                @endif
                            @endif
                                    ...
                                    @if ($user->category->id == 2)
                                        @if (!$user->parents->isEmpty())
                                            @if (!$user->parents->where('id', Auth::user()->id)->first()->pivot->verified)
                                                <a class="btn btn-secondary float-right mx-1" id="verifyBtn{{ $user->id }}"
                                                    href="#"
                                                    onclick="verifyRelAjax({{ $user->id }})">{{ __('Verify') }}</a>
                                            @else
                                            ...
                                            @if (!$user->parents->where('id', Auth::user()->id)->first()->pivot->freezed)
                                                <a class="btn btn-danger float-right mx-1" id="freezeBtn{{ $user->id }}"
                                                    href="#"
                                                    onclick="freezeRelAjax({{ $user->id }}, true)">{{ __('Freeze') }}</a>
                                            @else
                                                <a class="btn btn-info float-right mx-1" id="freezeBtn{{ $user->id }}"
                                                    href="#"
                                                    onclick="freezeRelAjax({{ $user->id }}, false)">{{ __('Unfreeze') }}</a>
                                            @endif
                                        @endif
                                    @endif
                                @endif
                                ...
                                @if (Auth::user()->category->id == 4)
                                    @if ($user->category->id != 5)
                                        @if (!$user->children->isEmpty() && $user->children->contains('id', Auth::user()->id))
                                            ...
                                            @endif
                                        @else
                                            <a class="btn btn-secondary float-right mx-1" id="addBtn{{ $user->id }}"
                                                href="#"
                                                onclick="addParentAjax({{ $user->id }})">{{ __('Add') }}</a>
                                        @endif
                                    @endif
                                @endif
                                @if (Auth::user()->category->id == 2)
                                    @if ($user->category->id == 1)
                                        @if (!$user->children->isEmpty() && $user->children->contains('id', Auth::user()->id))
                                            ...
                                            @endif
                                        @else
                                            <a class="btn btn-secondary float-right mx-1" id="reqBtn{{ $user->id }}"
                                                href="#"
                                                onclick="reqRelAjax({{ $user->id }})">{{ __('Request') }}</a>
                                        @endif
                                    @endif
                                @endif
