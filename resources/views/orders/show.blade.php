...
@section('content')
    ...
    <div class="container py-4">
        @auth
            <h2>{{ __('Order') }}</h2>
            <small class="text-muted">{{ __('Order Number') }}: {{ $order->id }} -
                {{ $order->created_at->format('d/m/Y') }}&nbsp; {{ $order->created_at->format('H:i:s') }}</small>
            @if (Auth::user()->user_category_id == 6 || Auth::user()->id == $order->reciever->id)
                ...
                <div>
                    <div class="row col-12 p-0">
                        ...
                        <div class="col-md-3">
                            ...
                            <p>
                                {{ $order->sender->itsCountry->ar_name }}, {{ $order->sender->itsCity->ar_name }},
                                @if ($order->sender->region != null)
                                    {{ $order->sender->region }},
                                    @endif @if ($order->sender->address != null)
                        ...
                        <div class="col-md-3">
                            ...
                            <p>
                                {{ $order->sender->mob1 }} @if ($order->sender->tel1 != null)
                                    - {{ $order->sender->tel1 }}
                                @endif
                            </p>
                        </div>
                            ...
                            <tbody>
                                @foreach ($order->items as $item)
                                    ...
                                    <tr>
                                        ...
                                        <td colspan="3">{{ __('Reciever Remark') }}: {{ $item->pivot->reciever_remark }}
                                        </td>
                                        <td colspan="2" id="trow">
                                            @if (Auth::user()->id == $order->reciever->id && $order->verified_at == null && $item->pivot->verified_at == null)
                                                <a class="btn btn-sm btn-secondary" title="{{ __('Insert Reciever Remark') }}"
                                                    href="#"
                                                    onclick="openVerItModal({{ $order->id }},{{ $item->id }})"><i
                                                        class="fas fa-comment"></i></a>
                                                <a id="verOrdItmBtn{{ $order->id }}{{ $item->id }}"
                                                    class="btn btn-sm btn-secondary" href="#"
                                                    onclick="verItemInOrder({{ $order->id }},{{ $item->id }})">{{ __('Verify') }}</a>
                                ...
                                @foreach ($order->baskets as $item)
                                    <tr data-toggle="collapse" class="clickable"
                                        data-target="#collapseBItems{{ $item->id }}" aria-expanded="false"
                                        aria-controls="collapseBItems{{ $item->id }}">
                                        ...
                                        <td colspan="9">
                                            <div class="collapse" id="collapseBItems{{ $item->id }}">
                                                @foreach ($item->items as $itemm)
                                                    {{ $itemm->name . ' ' . $itemm->type->ar_name . ' ' . $itemm->titer }} :
                                                    <b>{{ $itemm->pivot->quantity }}</b>
                                                    <br>
                                                @endforeach
                                            </div>
                                        </td>
            ...
            <div class="mb-4" id="btns">
                @if (Auth::user()->user_category_id == 6 ||
                        Auth::user()->id == $order->sender->id ||
                        Auth::user()->id == $order->reciever->id)
                    @if (Auth::user()->id == $order->reciever->id && $order->verified_at == null)
                        <form action="{{ route('verifyOrder', ['id' => $order->id]) }}" method="POST" class="ml-1 float-right"
                            id="verForm">
                            @csrf
                            @method('PUT')
                        ...
                        <button class="btn btn-secondary float-right" data-toggle="modal"
                            title={{ __('Insert Reciever Remark') }} data-target="#sender_remark">
    ...
    <div class="modal fade" id="reciever_remark_item" tabindex="-1" role="dialog"
        aria-labelledby="reciever_remark_item_Label" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-secondary" data-dismiss="modal"
                        onclick="verItemInOrder()">{{ __('Verify') }}</button>
                </div>
            </div>
        </div>
    </div>
    <!-- Modal -->
    <div class="modal fade" id="sender_remark" tabindex="-1" role="dialog" aria-labelledby="reciever_remarkLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __('Close') }}</button>
                    <form action="{{ route('verifyOrder', ['id' => $order->id]) }}" method="POST"
                        class="mr-1 float-right" id="verForm">
                        @csrf
                        @method('PUT')
